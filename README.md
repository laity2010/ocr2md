...
数据表共用 `多选`、`行号`、`行类型`、`预览` 四列，默认按行号升序。单击“行号”“行类型”或“预览”可设为主排序；按住 `Shift` 再单击其他列可追加多列排序，箭头后的数字表示排序优先级。工作稿相对原件的新增、修改、删除行会着色；行号跟随工作稿插行、删行同步调整。点击预览时，编辑器左侧约 2/3、右侧约 1/3；右上为源码并定位到该行，右下为同一文件的 Markdown Preview。行号跟随、增删改着色和右窗布局都是数据表统一特性，标题、注释、图片模块同样适用。

注释模块在共用表之上增加自己的 `注释号` 列，默认按注释号、再按行号升序。该列不是数据表的一般列。提取不到注释号的行会着色提示手工输入；“匹配注释对”按注释号配对引用和正文，并标出待补引用或待补正文。章节文件、注释配对状态与图片本地路径仍显示在预览单元格内。
