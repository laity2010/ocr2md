...
数据表固定为 `多选`、`行号`、`行类型`、`预览` 四列。单击“行号”“行类型”或“预览”可设为主排序；按住 `Shift` 再单击其他列可追加多列排序，箭头后的数字表示排序优先级。章节文件、注释号/配对状态与图片本地路径显示在预览单元格内。提取不到注释号的行会着色提示手工输入；“匹配注释对”按注释号配对引用和正文，并标出待补引用或待补正文。
