...
- **嵌入块**：单独的 `>` 行列为「嵌入块首」；图片、HTML、内嵌标题和紧挨着的说明共用同一序号，直到下一个 `>`。章节正文不会收成嵌入文本。行类型为嵌入块首、内嵌标题、嵌入链接、嵌入 HTML、HTML 表、嵌入文本。数据表显示序号。「下载图片到本地」会下载全部外部图片，并把状态写入标定以便断点续传。VS Code 粘贴产生的本地 Markdown 图片会预先显示实际导出路径，并在「按标定导出」时复制到章节 `imgs/`，使用带文件修改时间戳的唯一文件名，原图和工作稿不改动。「按标定导出」按标题、注释、嵌入块规则写入 `chapters/章节目录/output/`。
