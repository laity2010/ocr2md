...
数据表共用 `多选`、`行号`、`行类型`、`预览` 四列，默认按行号升序。单击“行号”“行类型”或“预览”可设为主排序；按住 `Shift` 再单击其他列可追加多列排序，箭头后的数字表示排序优先级。工作稿相对原件的新增、修改、删除行会着色；行号跟随工作稿插行、删行同步调整。这两项是数据表统一特性，标题、注释、图片模块同样适用。
