...
- **嵌入块**：找出章节中的图片链接和 HTML 代码，行类型为内嵌标题、嵌入链接、嵌入 HTML、嵌入文本，并支持下载外部图片。

各模块的数据表统一支持 `已删除` 软标记。被标记的记录会保留用于审计，但不会参与订正、配对、导出或图片下载。

数据表共用 `多选`、`行号`、`行类型`、`预览` 四列，默认按行号升序。单击“行号”“行类型”或“预览”可设为主排序；按住 `Shift` 再单击其他列可追加多列排序，箭头后的数字表示排序优先级。工作稿相对原件的新增、修改、删除行会着色；行号跟随工作稿插行、删行同步调整。点击预览时，侧栏表格留在左侧，右侧编辑区上半为源码并定位到该行，下半为同一文件的 Markdown Preview。行号跟随、增删改着色和右窗布局都是数据表统一特性，标题、注释、嵌入块模块同样适用。
